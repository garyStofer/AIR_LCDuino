...
WindDirCal( void )
{
   ...
   unsigned char pressCnt = ShortPressCnt;
   
   WindCal.WDir_min = 0xffff;
   WindCal.WDir_max = 0;

   while ( pressCnt == ShortPressCnt )
   {
...
// TODO: calibrate the offset 
// for now left at 0 -- confuses people less
...
void WindRead()
{
  ...
  // Precaution in case the calib values are wrong 
  if (WindDir > 360)
    WindDir = 360;
  else if (WindDir < 0)
    WindDir = 0;
